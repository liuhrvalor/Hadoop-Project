...
public class Multiplication {
	...
	public static class MultiplicationReducer extends Reducer<Text, Text, Text, DoubleWritable> {
		...
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			// all these code are associated with movieB( xxB * movieB + xxC * C)
			double sum = 0.0;
			for(Map.Entry<String, Double> entry: relationMap.entrySet()){
				...
				for(Map.Entry<String, Double> entry1 : ratingMap.entrySet()){
					...
					String outputString = user + ":" + movieA;

					context.write(new Text(outputString), new DoubleWritable(outputValue));
